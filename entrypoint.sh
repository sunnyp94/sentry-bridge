#!/bin/sh
# When DISCOVERY_ENABLED=true: run discovery 8:00–9:30 ET (every 5 min), then at market open start the engine.
...
if [ -n "$ACTIVE_SYMBOLS_FILE" ]; then
  if [ "$DISCOVERY_ENABLED" = "true" ] || [ "$DISCOVERY_ENABLED" = "1" ]; then
    echo "[entrypoint] discovery phase 8:00–9:30 ET; then starting engine at market open"
